...
Program Nicole
  ...
  Integer :: ierr, NotifyWhenFinished, request
  External :: CheckNaN
  myrank=0
  nprocs=1
  !
  ...
  Read (headerunit, *) Params%formal_solution
  ...
  If (myrank .eq. 0) then
     ...
     If (Restart .eq. -1) then
        ...
        If (FileExists) then
           ...
           Close (tmpunit, STATUS='DELETE')
        End if
        Inquire (File=Trim(Input%Syn_profile_file)//'.err',Exist=FileExists)
        If (FileExists) then
  ...
  If (myrank .eq. 0) then ! Master block. Loop to send jobs to slaves and wait for response
     ...
     Do While (MaxVal(Pixel_to_do) .eq. 1 .or. MaxVal(host_busy(1:)) .eq. 1)
        ...
        If (MaxVal(Pixel_to_do) .eq. 1 .and. MinVal(host_busy) .eq. 0) then 
           ...
           End if
           Call Record_to_model_2comp(Params%n_points, Guess_model, TmpModel, TmpModel2, 1)
...
Contains
  Subroutine Do_Task
    Call Record_to_Model_2comp(Params%n_points, Guess_model, TmpModel, TmpModel2, 1) ! Extract model from array
    ix=(irec-1)/nPix_y +1
    iy=Modulo(irec-1,nPix_y) +1  
