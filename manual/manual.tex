...
asterisks with exclamation marks.
...
indentation. The various sections and the lines within each section
...
\item Acceptable Chi-square: (Optional, default=5). This parameter is
...
When doing inversions, we can have a section specifying the number of
nodes to be used. It starts with the line:
...
Temperature, T, Temp, Velocity, v, vel, vlos, v\_los, Microturbulence,
micro, vmic, v\_mic, Macro, v\_mac, B\_long, Blong, Bl, Bz,
B\_x, Bx, B\_y, By, Stray, Stray light, Expansion, Exp, Abundances,
Number of abundances. NICOLE has defaults for all of these values so
you don't need to specify them all. Here is an example of such
a section:
...
filling factor. If there is a third number in ths line, it is taken as
the magnetic field expansion parameter (between 0 and 1). Otherwise,
expansion is set to zero.

The field expansion parameter is used to account for the fact that one
expects the filling factor to be larger in the chromosphere than in
the photosphere. A rigorous treatment of this dependence would require
computing many rays in the resolution element, piercing the atmosphere
from the center of the magnetic element all the way out to the
non-magnetic surroundings. This would result in very high
computational demands. Instead, we make the simple approximation that
some particular wavelength ranges are affected by the {\it
photospheric} filling factor, whereas other ranges are affected by the
{\it chromospheric} filling factor. This works well because the
transition from photospheric to chromospheric spectral regions is
usually very sharp. For example, the polarization signal in the wings
of the Ca infrared triplet lines is purely photospheric, while the
signal in the line core is chromospheric. The transition between the
two regimes is very sharp, so we can use the photospheric filling
factor for the wings and the chromospheric filling factor for the
core. The expansion parameter indicates how much of the pixel that was
field-free in the photosphere is magnetic in the chromosphere. If the
expansion is zero, it means that the chromospheric filling factor is
the same as the photospheric one. If it's one, it means that the
entire chromosphere is magnetic, regardless of the photospheric
filling factor.
...
expansion. Finally, the array abundance (nx,ny,92) can be used to
...
stray\_frac, expansion, and the keep flags). If you are having trouble
...
stray\_frac, expansion, keep\_el\_p, keep\_gas\_p, keep\_rho, 
...
NICOLE.input to output some further information:
\begin{itemize}
...
... and at that point I stopped keeping track
