...
; Parse data
      ...
      wlength=strmid(str,7,12)
      wlength=strtrim(wlength,2)
      loggf=strmid(str,20,7)
      loggf=strtrim(loggf,2)
      pot=strmid(str,28,8)
      pot=strtrim(pot,2)
      jlow=strmid(str,37,5)
      jlow=strtrim(jlow,2)
      jup=strmid(str,52,5)
      jup=strtrim(jup,2)
      landelow=strmid(str,58,6)
      landelow=strtrim(landelow,2)
      landeup=strmid(str,65,6)
      landeup=strtrim(landeup,2)
      gamma_r=strmid(str,79,6)
      gamma_r=strtrim(gamma_r,2)
      gamma_stk=strmid(str,86,6)
      gamma_stk=strtrim(gamma_stk,2)
      gamma_vdw=strmid(str,93,6)
