...
module background_opacity_module
  ...
  use debug_module
  implicit none
  Integer :: Opacity_package=1 ! Default = 1, may be changed by caller
contains
  ...
  function background_opacity(T4, Pe4, Pg4, PH4, PHminus4, PHplus4, PH24, PH2plus4, lambda_in4, Scat)
    real :: background_opacity, T4, Pe4, Pg4, PH4, PHminus4, PHplus4, PH24, PH2plus4, lambda_in4, Scat
...
!    flag=0
    If (Opacity_Package .eq. 2) then ! Use SOPA 
       nu=cc/(lambda_in4*1e-8) ! s^-1
       Call Sopas(1, 2, nu, Pe4, T4, Pg4, chi_0, chi_e, eta)
       Background_opacity=chi_0+chi_e
       Scat=chi_e
!       if (background_opacity .gt. 1e-3) then
!          flag=1
!          print *,'****** T=',T4,' elp=',Pe4,' Pg4=',Pg4,' lambda=',lambda_in4
!          print *,'sopa=',background_opacity,scat
!       endif
       Return
    endif
    ...
    If (lambda_in .lt. Min_Lambda) then
       Call Debug_log('In background_opacity. Lambda .lt. Min_Lambda parameter. Clipping it.',2)
       lambda_in=Min_Lambda
    End if
    If (lambda_in .gt. Max_Lambda) then
       Call Debug_log('In background_opacity. Lambda .gt. Max_Lambda parameter. Clipping it.',2)
       lambda_in=Max_Lambda
    End if
    If (T .lt. Min_Temp) then
       Call Debug_log('In background_opacity. Temperature .lt. Min_Temp parameter. Clipping it.',2)
       T=Min_Temp
    End if
    If (Pe .lt. Min_Pe) then
       Call Debug_log('In background_opacity. Pe .lt. Min_Pe parameter. Clipping it.',2)
       Pe=Min_Pe
    End if
    If (T .gt. Max_Temp) then
       Call Debug_log('In background_opacity. Temperature .gt. Max_Temp parameter. Clipping it.',2)
       T=Max_Temp
    End if
    If (Pe .gt. Max_Pe) then
       Call Debug_log('In background_opacity. Pe .gt. Max_Pe parameter. Clipping it.',2)
       Pe=Max_Pe
    End if
    ...
    Scat=contrib4+contrib5+contrib6
    
!    if (flag .eq. 1) print *,'andr=',background_opacity,scat
    ...
    If (Scat .lt. 0) then
      ...
      Call Debug_log('In background_opacity. Scat .lt. 0. Clipping it.'//String,2)
       Scat = 0.
    End if
   
    If (background_opacity .lt. 0.) then
       Write (String,*) 'T, Pe, PH, PHminus, PHplus, PH2, PH2plus, lambda=', &
            T4,Pe4, PH4, PHminus4, PHplus4, PH24, PH2plus4, lambda_in4
      Call Debug_log('In background_opacity. Opac .lt. 0. Clipping it.'//String,2)
       background_opacity = 0.
    End if
   
    If (background_opacity .gt. 1.e30) then
       Write (String,*) 'T, Pe, PH, PHminus, PHplus, PH2, PH2plus, lambda=', &
            T4,Pe4, PH4, PHminus4, PHplus4, PH24, PH2plus4, lambda_in4
       Call Debug_log('In background_opacity. Opac .gt. 1.e30 Clipping it'//String,2)
       background_opacity = 1.e30
    End if
