...
! From here on may be needed for repeated calls with different atmospheres
  ...
  If (NLTEInput%Hydro) then
     Call Hydrostatic(Params, NLTE%Atmo, Error) ! To fill in some arrays in Atmo
  ...
  End if
  If (FirstTime) NLTE%N(:,:)=NLTE%NStar(:,:) ! Populations initial guess
  FirstTime=.FALSE.
...
!
 ...
 End if ! End multi routine rcoll
!
...
! Allocate (C2(Atom%NK, Atom%NK, NLTE%NDEP))
! print *,'debug 2b in nlte.f90'
! debug
! open (84,file='../run/col.dat')
! do k=1,ndep
!    do i=1,atom%nk
!       do j=1,atom%nk
!          read (84,*) c2(i,j,k)
!
!          c(i,j,k)=c2(i,j,k) ! use MULTI collisions
!
!       end do
!    end do
! end do
...
! debug
!      print *,'debug in nlte.f90, lte_pop_2'
!      print *,'reading rho.txt generated by multi'
!      open (94,file='../run/rho.txt')
!      read (94,*) n_tot
!      read (94,*) n_tot
!      read (94,*) nlte%atmo%nh
!      read (94,*) nlte%atmo%nhplus
!      close (94)
...
!
           ...
           If (Cont) then ! b-f transitions
              Jeff(:)=Jnu(:)-LStar(:)*NLTE%Sl(:,itran)*.99
